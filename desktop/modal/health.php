...
/*
	DEPLACE DANS 	public static function ScanAmazonAlexa($_logical_id = null, $_exclusion = 0) de core class alexatodolist.class.php
...
foreach($json as $item)
{
		...
		if (!is_object($alexatodolist)) {
			$alexatodolist = new alexatodolist();
			$alexatodolist->setName($device);
			$alexatodolist->setLogicalId($serial); 
			$alexatodolist->setEqType_name('alexatodolist');
			$alexatodolist->setIsEnable(1);
			$alexatodolist->setIsVisible(1);
		}
		$alexatodolist->setConfiguration('serial',$serial); 
		$alexatodolist->setConfiguration('device',$device);
